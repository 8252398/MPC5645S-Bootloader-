...
extern void main();
extern void Bootloader_Function(void);
extern void CopyBootloaderCodeToRAM(void);
...
__declspec(section ".init") extern void __startd(int argc, char **argv, char **envp);				/* primary entry point */
 ...
 */
/***************************************************************************/
asm void __startd(register int argc, register char **argv, register char **envp)
{
	...
	// copy bootloader into ram
/**/	
	#if FALSE
	lis      r6,CopyBootloaderCodeToRAM@ha
	addi     r6,r6,CopyBootloaderCodeToRAM@l
	mtlr     r6
	mr		 r3, r14
	mr		 r4, r15
	mr		 r5, r16 
	...
	//branch to bootloader program
	lis      r6,Bootloader_Function@ha
	addi     r6,r6,Bootloader_Function@l
	mtlr     r6
	mr		 r3, r14
	mr		 r4, r15
	mr		 r5, r16
 ...
 */
/***************************************************************************/
extern void __copy_rom_sections(void* dst, const void* src, unsigned long size)
{
	if (size && (dst != src)) {
		memcpy(dst, src, size);
 ...
 */
/***************************************************************************/
extern void __init_bss_sections(void* dst, unsigned long size)
{
	if (size) {
		memset(dst, 0, size);
	}
...
extern void __init_data(void)
{
	...
	__init_bss_sections(__fsl_bss_start, __fsl_bss_end - __fsl_bss_start);
	...
	while (1) {
		if (dci->rom == 0 && dci->addr == 0 && dci->size == 0) break;
 		__copy_rom_sections(dci->addr, dci->rom, dci->size);
 		dci++;
	}
	...
	while (1) {
		if (bii->addr == 0 && bii->size == 0) break;
 		__init_bss_sections(bii->addr, bii->size);
 		bii++;
	}
#endif
}
